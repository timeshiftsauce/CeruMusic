...
try {
  ...
  await page.route('**/src/services/pluginState.ts*', route => route.fulfill({contentType:'text/javascript',body:`import {ref} from '${vueModule}';export const pluginContributions=ref(window.fixtures.map(p=>({pluginId:p.pluginId,manifest:p.manifest,enabled:p.enabled})));export const refreshPluginContributions=async()=>{};`}))
  ...
  await page.goto(baseURL + '/__plugin_design_preview')
  ...
  const before=await page.locator('.plugin-item').nth(1).boundingBox()
  await page.locator('.plugin-item').nth(1).getByRole('button',{name:'使用',exact:true}).click()
  await page.locator('.plugin-item').nth(1).getByRole('button',{name:'关闭',exact:true}).waitFor()
  const after=await page.locator('.plugin-item').nth(1).boundingBox()
  assert.equal(after.height,before.height,'changing state does not resize the row')
  await page.getByRole('button',{name:'更多插件操作'}).first().click()
  assert.equal(await page.getByText('更新插件',{exact:true}).count(),0)
  await page.keyboard.press('Escape')
  await page.locator('h2').click()
  await page.locator('.t-message').waitFor({state:'hidden'})
  const children=page.locator('.plugin-item').filter({has:page.locator('.format-badge', {hasText:'LX'})})
  assert.equal(await children.count(),2,'guests are independent list entries')
  assert.equal(await children.first().locator('.format-badge').evaluate(el=>getComputedStyle(el).backgroundColor),'rgb(22, 135, 93)')
  await page.locator('.plugin-item').nth(1).getByRole('button',{name:'关闭',exact:true}).click()
  await page.locator('.plugin-item').nth(1).getByRole('button',{name:'使用',exact:true}).waitFor()
  assert.equal(await children.count(),2,'closed dependency does not hide installed children')
  await children.first().getByRole('button',{name:'使用',exact:true}).click()
  await children.first().getByRole('button',{name:'关闭',exact:true}).waitFor()
  await children.nth(1).getByRole('button',{name:'使用',exact:true}).click()
  await children.nth(1).getByRole('button',{name:'关闭',exact:true}).waitFor()
  assert.equal(await children.getByRole('button',{name:'关闭',exact:true}).count(),1)
  await page.locator('.t-message').first().waitFor({state:'hidden'})
  const screenshot=join(tmpdir(),'ceru-plugin-page.png')
  await page.screenshot({path:screenshot,fullPage:true})
  await page.setViewportSize({width:660,height:720})
  assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),'no horizontal overflow')
  assert.deepEqual(errors,[])
  console.log(JSON.stringify({screenshot,checks:'Vue rendering, use/close, stable row height, no manual update, compact viewport'}))
